...
AddEventHandler('phone:assistNotify', function(id, service, player)
  TriggerEvent('jobssystem:current', function(job)
    ...
    elseif (job == 'police' or job == 'news') and (service == 'AIcall26' or service == 'AIcall25' or service == 'AIcall24' or service == 'AIcall23' or service == 'AIcall' or service == 'AIcall2' or service == 'AIcall3a' or service == 'AIcall3' or service == 'AIcall4' or service == 'AIcall5' or service == 'AIcall6' or service == 'AIcall7' or service == 'AIcall8' or service == 'AIcall17' or service == 'AIcall18' or service == 'AIcall19' or service == 'AIcall20' or service == 'AIcall21' or service == 'AIcall22' or service == 'AIcall9') then
      if job == 'news' then
        addBlip({service = service, player = player, id = id,jobType = "news"})
        local radiocount = exports["np-inventory"]:getQuantity(59)
...
function GetClosestPlayers()
  ...
  for index,value in ipairs(players) do
    ...
    if(target ~= ply) then
      ...
      if(distance < 25) then
        table.insert(closestplayers,target)
      end
    end
  end
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    elseif exports["np-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
      local dist = 0
      if lastBlip.x then 
        dist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),lastBlip.x,lastBlip.y,lastBlip.z)
      end
...
function nearDOJ()
  ...
  for _,k in pairs(nearDOJPlaces) do
    ...
    local distance = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), k[1],k[2],k[3], true)
  ...
  while true do
    ...
    for key, item in pairs(myBlips) do
      if (key ~= nil and item ~= nil) then
        ...
        if GetDistanceBetweenCoords(pos, item.pos.x, item.pos.y, item.pos.z, true) < 10.0001 then
...
--[[   if GetPlayerPed(-1) == GetPlayerPed(GetPlayerFromServerId(tonumber(data.player.source))) then
    return
  end ]]
  local pos = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(tonumber(data.player.source))))
  ...
  myBlips[data.id] = {service = data.service, timestamp = GetGameTimer(), loc = loc, onRoute = false, pos = pos, blip = blip, id = data.id,jobType = data.type}
